...
2. Here, we develop a new method that allows to keep the raw tree ring measurements and individual tree's metrics to simultaneously model non-linear effects of objectively determined principle climate drivers and e.g. tree diameter. 
...
We then include these in generalized least squares models to model tree rings (or derived measurements) while accounting for the temporal autocorrelation inherent to each individual tree's growth.
We apply this method to tree-ring data from **#** species at ten globally distributed sites spanning a wide range of forest types. 
