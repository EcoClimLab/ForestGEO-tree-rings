...
However, tree size and time are almost inextricably linked (because each individual core increases in $DBH$ through time) and therefore difficult to disentangle.  
A variety of methods have been used to analyze directional changes in grwoth rate through time, including "conservative detrending", where a mathematical function () is fit to growth trends in each individual core and residuals analyzed for trends and "basal area correction", which assumes that the growth in a tree's basal area ($BAI$) is fundamentally independent of size [@peters_detecting_2015].
None of these methods are fully satisfactory, being subject to various sampling and analysis biases [@cherubini_potential_1998; @nehrbassahles_influence_2014; @peters_detecting_2015], leaving great uncertainty as to what the tree-ring record can tell us about the impact of slowly changing environmental drivers such as rising CO~2~ [@sullivan_effect_2016; @walker_integrating_2020]. 
To disentangle $DBH$ from time and detect directional grwoth trends, it is necessary to simultaneously account for the influences of changing tree size and environmental drivers, including climate.
...
We note that a simple modification of our analysis method (modeling $year \times$ climate interactions instead of $climate \times$ climate interactions) could be used to account for directional changes in climate sensitivity, which have commonly been observed [REFS- Maxwell, Helcoski; @peltier_tree_2020; @zuidema_recent_2020].
...
Growth rates changed directionally for the majority (`r percent_sig_neg_trend_of_analyzed`, depending on growth metric) of site-species combinations that could be analyzed (Fig. `r year_fig`).
This finding was consistent with an independent analysis (Appendix `r decadal_analysis_appendix`, Figs. `r decadal_analysis_figs_all`) and corroborates previous analyses from HKK showing growth decreases over time [@nock_longterm_2011; @van_der_sleen_no_2015; @groenendijk_no_2015, but see @brienen_tree_2017].
*(have there been analyses of this at any other sites?)*
Although $DBH$ and $year$ are difficult to disentangle [e.g., @peters_detecting_2015; @brienen_tree_2017], decent coverage of $DBH$ x $year$ matrices for the species analyzed, together with general consistency across growth metrics and analysis methods (Appendix `r decadal_analysis_appendix`, Figs. `r year_fig`, `r decadal_analysis_figs_all`) suggest that our approach is relatively reliable, *albiet* still subject to the influence of some potential sampling and survivorship biases [@brienen_detecting_2012; @brienen_tree_2017], evaluation of which is beyond the scope of the current analysis.
At a minimum, it provides a more appropriate alternative to two of the most commonly used methods for analyzing growth trends, conservative detrending and basal area correction [see @peters_detecting_2015], which would fail to capture the observed trends in $RW$ and $BAI$ with $DBH$ (Fig. `r size`).
...
Indeed, in a comparative analysis of methods to detect growth trends, @peters_detecting_2015 found that approach similar to ours ("regional curve standardization") to be among the most robust and recommended that future analyses take an approach very similar to that used here. 
...
The majority of negative growth trends observed here are probably attributable to increasing competition as secondary stands age. 
