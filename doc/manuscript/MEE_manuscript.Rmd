...
We ultimately determined that the method described above was reliable for the purposes of this analysis (Appendix `r changing_climate`).
...
The relative importance of climate was modest (but still significant) at sites including SCBI (Fig. `r GLS_SCBI`), HF (Fig. `r GLS_HF`), and SC (Fig. `r GLS_SC`), and stronger at sites including BCI (Fig. `r GLS_BCI`) and LT (Fig. `r GLS_LT`). 
...
n_DBHxP_RW_sig=13
n_DBHxP_RW_sig_pos=13-3 #(exceptions: BEAL & TSCA at HF, AFXY at HKK)
n_DBHxP_BAI_analyzed= 38
n_DBHxP_BAI_sig=18
n_DBHxP_BAI_sig_pos=18-6 #(exceptions: QURU & ACRU at HF, BEPA at NIO, QUVE at SCBI,  ACSA at LDW, AFXY at HKK)
#T:
n_DBHxT_RW_analyzed= 34
n_DBHxT_RW_sig=13
n_DBHxT_RW_sig_neg=5 #notable that it's minority neg for RW, contrasting with slight majority neg for BAI 
n_DBHxT_BAI_analyzed= 35
n_DBHxT_BAI_sig=10
n_DBHxT_BAI_sig_neg=6 #mostly drought signal
```
...
For precipitation variables, interactions were significant for `r round(n_DBHxP_RW_sig/ n_DBHxP_RW_analyzed*100)`% of interactions with $RW$ as the growth metric and for `r round(n_DBHxP_BAI_sig/ n_DBHxP_BAI_analyzed*100)`% with $BAI$ as the growth metric (Table `r interactions_and_year_summaries`).
...
Among the exceptions to this pattern (n=`r n_DBHxP_RW_sig-n_DBHxP_RW_sig_pos` for $RW$, `r n_DBHxP_BAI_sig-n_DBHxP_BAI_sig_pos` for $BAI$), only a minority (n=1 for $RW$, 3 for $BAI$) occurred in species responding positively to precipitation in the current growing season. 

Temperature variable $\times DBH$ interactions were significant for `r round(n_DBHxT_RW_sig/ n_DBHxT_RW_analyzed*100)`% of cases with $RW$ as the growth metric and for `r round(n_DBHxT_BAI_sig/ n_DBHxT_BAI_analyzed*100)`% with $BAI$ as the growth metric (Table `r interactions_and_year_summaries`).
...
```{r , eval=TRUE, echo=FALSE}
## Tallies for the  paragraph below (Dec.27)
n_tested=40 #same for all metrics
n_best_model_RW=32
n_best_model_BAI=33
n_best_model_AGB=36
n_best_model_neg_RW=32-2 #(exceptions:  FASY at ZOF, PIPU at CB) (94%)
n_best_model_neg_BAI=33-3 #(exceptions:  FASY at ZOF, PIPU and POTR at CB) (91%)
n_best_model_neg_AGB=36-3 #(exceptions:  FASY at ZOF, PIPU and POTR at CB) (92%)
```

*(tentative results.  We're not yet getting the constraints on scope right. It is messing up on POTR, for which only ~20 yrs of data get included.)*

There was a significant effect of `year` in the GLS models for `r n_best_model_RW` - `r n_best_model_AGB` (depending on growth metric) of the `r n_tested` species- site combinations tested (Table `r interactions_and_year_summaries`), and these trends matched those observed in a separate analysis of $DBH$-growth relationships by year (Appendix `r decadal_analysis_appendix`; **ADD SI FIGS**).
In the majority of cases (91-94%, depending on growth metric), the growth trend over time was negative.
...
Significant positive growth trends were observed for only three species, FASY at ZOF, PIPU and **POTR** at CB, and all were modest compared to the steep negative trends observed for some species.
Growth rate was consistently independent of `year` for only four species: *Chukrasia tabularis* at HKK, *Fraxinus nigra* at SCBI, and *Picea engelmannii* and *Pinus flexilis* at CB.

![**Figure 6 | DRAFT FIGURE. GLS model with and without year, maybe decadal analysis plot. IT WOULD BE BEST TO COMBINE THE 2 DBH FIGURES (PANELS 1,2). CURRENT EXAMPLE IS JUST SCOTTY CREEK, BUT IT WOULD BE GOOD TO SHOW AT LEAST ONE OTHER.](tables_figures/year_figure_sketch.png)
