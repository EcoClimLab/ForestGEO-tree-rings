...
[**(figure or table on climate - DBH interactions-- see ISSUE #107)**](https://github.com/EcoClimLab/ForestGEO-climate-sensitivity/issues/107)
...
n_best_model_neg_BAI=n_best_model_BAI-3 #(exceptions:  FASY at ZOF, PIPU and PIFL at CB) 
n_best_model_neg_AGB=n_best_model_AGB-3 #(exceptions:  FASY at ZOF, PIPU and PIFL at CB) 
#species consistently independent of year: PIEN and PILO at CB, CHTA at HKK, PIST3 at NM, 

```

There was a significant effect of `year` in the GLS models for `r n_best_model_RW` - `r n_best_model_AGB` (depending on growth metric) of the `r n_tested` species- site combinations tested (Fig. `r year_fig`), and these trends were consistent with those observed in a separate analysis of $DBH$-growth relationships by year (Appendix `r decadal_analysis_appendix`; **ADD SI FIGS**).
