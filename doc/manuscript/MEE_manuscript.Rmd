...
Growth responses were predominantly positive to precipitation and negative to temperature, with both included in 78% of top models, and with non-linear responses prevalent (*>50%* of relationships).
...
Yet, collection and analysis of dendrochronological records has traditionally been optimized to detect climate signals rather than to predict forest productivity, its climate sensitivity, and how it may be changing [@cherubini_potential_1998; @nehrbassahles_influence_2014; @klesse_sampling_2018;@babst_when_2018; @wilmking_global_2020].
...
In particular, analysis of species-level chronologies does not allow characterization of known variation in climate sensitivity in relation to tree size, canopy position, or microhabitat [e.g., @bennett_larger_2015; @mcgregor_tree_2020; @rollinson_climate_2021].
...
Tree diameter at breast height ($DBH$) is closely correlated with numerous variables affecting tree growth rate [e.g., height, crown size and position, root mass, @enquist_global_2002] and therefore is one of the most important variables influencing growth [e.g.,@foster_predicting_2016;  @muller-landau_testing_2006] and its climate sensitivity [e.g., @bennett_larger_2015; @mcgregor_tree_2020].
...
For example, larger trees tend to be relatively more sensitive to drought [@bennett_larger_2015; @gillerot_tree_2020; @mcgregor_tree_2020], and responses to temperature can also vary with tree size [@rossi_age-dependent_2007; @rollinson_climate_2021].
...
To use tree-rings to predict tree growth, $ANPP_{woody}$, and forest dynamics, we need models that include $DBH$ [e.g., @evans_fusing_2017; @klesse_amplifying_2020] and its potential climate interactions [e.g., @rollinson_climate_2021].

Characterizing how tree growth and forest productivity are responding to slowly-changing environmental drivers is very challenging and uncertain.
...
(4) How have growth rates of contemporary cohorts of trees changed through time?
...
**Table 1 | Sites included in this analysis**. Here and throughout, sites are ordered by descending mean annual temperature. Additional site information is provided in Appendix `r site_details_appendix` and Table `r site_details`, and tree species and sampling details are detailed in Tables `r species_table`-`r sampling_details_table`. 
...
From among the full set of original $RW$ measurements, we excluded cores for which we detected errors (e.g., labeling inconsistencies, obvious dating errors) that could not be resolved before finalizing the analysis. 
...
As an example of a full model for $log[RW]$ would look like this in *R*: 
...
Finally, in two cases of variables that had only weak effects and mixed responses among species in the final models (temperature variable group at HKK, precipitation variable group at HF), *climwin* identified different climate variables and different time windows (Figs. `r climwin_HKK_T`, `r climwin_HF_T`). 
...
In the extratropics, the most influential windows were limited to the current spring and early summer at LDW, whereas optimal windows at three sites extended back to the previous fall (LT, CB) or summer (SCBI).
...
An analytical structure that can account for this and other such DBH-climate interactions [e.g., @rossi_age-dependent_2007; @rollinson_climate_2021] will be critical to using tree-ring records to understand and forecast the effects of climate on tree growth and forest productivity.
...
However, our results remain subject to the potential influence of some potential sampling and survivorship biases that can influence overall trends in positive or negative directions [Fig. `r year_fig`; @brienen_detecting_2012; @groenendijk_no_2015; @brienen_tree_2017; @nehrbassahles_influence_2014].
