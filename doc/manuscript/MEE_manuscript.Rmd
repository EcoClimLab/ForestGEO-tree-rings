...
*(need to describe GLS models better)*
...
For each species (at each site) independently, we ran a GLS model including every combination of the candidate climate variables (as a 2-degree polynomial to allow quadratic curve), a spline of $DBH$ (3 knots), a random intercept for each tree, and *a temporal autocorrelation matrix*. 
We considered the model with lowest AIC as the top model. 
We then tested the individual significance of each term in the top model, including both first- and second-order terms of polynomial fits to climate variables. To do so, we compared the AIC of the top model (containing the term) to a model lacking the term, counting as significant any term with $\Delta AIC \le$ 2.0.
