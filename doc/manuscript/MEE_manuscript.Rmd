...
This finding was consistent with an independent analysis of growth-$DBH$ trends by decade (Figs. `r year_fig`, `r decadal_analysis_figs_all`), and results were generally consistent regardless of the growth metric analyzed (Figs. `r all_GLS`), indicating that our approach is robust in its analytical structure.
Our results are, however, still subject to the influence of some potential sampling and survivorship biases [Fig. `r year_fig`; @brienen_detecting_2012; @brienen_tree_2017; @nehrbassahles_influence_2014], evaluation of which is beyond the scope of the current analysis.
...
Yet, the trends detected here remain potentially subject to a variety of sampling and survivorship biases that can influence overall trends in positive or negative directions (*REFS*) [Fig. `r year_fig`; @brienen_detecting_2012; @brienen_tree_2017].
...
Signals of changing growth rate over time are attributable to some combination of stand dynamics (e.g., recruitment and succession, changing stand structure) and environmental drivers (e.g., climate drivers other than those selected by *climwin*; atmospheric CO~2~; deposition of sulfur dioxide, SO~2~, and nitrogen oxides, NO~x~).
