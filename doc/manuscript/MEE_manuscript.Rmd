...
Below, we summarize how our results on the effects of climate, tree size, and year corroborate current understanding of the drivers of tree growth while yielding new insights made possible by this approach.
...
While our approach identifies similar climate sensitivities to those that would be identified using conventional methods (Figs. `r SCBI_example`, `r all_traditional_comparisons`;  Table `r qualitative_results_comparison`), the approach differs in some substantive ways.
...
We found that interactions between climate variables and $DBH$ were common (`r round(n_DBHx_analyzed_sig/ n_DBHx_analyzed*100)`%; Table `r interactions_and_year_summaries`, Fig. `r interactions_fig`).
The most coherent pattern observed in this analysis was a tendency for larger trees to be more sensitive to precipitation and high temperatures, consistent with widespread observations that larger trees are more sensitive to drought [REFS; @bennett_larger_2015; @gillerot_tree_2020; @mcgregor_tree_2020].
...
**Growth rate--whether measured as $RW$, $BAI$, or $\Delta AGB$-- varied nonlinearly with $DBH$, with the observed diversity of growth trends in relation to $DBH$ (Fig. `r size`) largely attributable to species ecology and stand history.**
...
It is notable that growth rates are changing directionally for most species/ sites.
