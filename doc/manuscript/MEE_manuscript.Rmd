...
For each species (at each site) independently, we ran a GLS model including every combination of the candidate climate variables (as a 2-degree polynomial to allow quadratic curve), $DBH$, a random intercept for each tree, and *a temporal autocorrelation matrix*. 
...
Responses were most commonly positive, and were most pronounced at the driest site (Little Tesuque). 
...
In some cases, the non-linearity was quite pronounced (e.g., Little Tesuque), with the most common pattern (#%) being a decelerating increase. 
...
Temperature terms included in top models were non-linear #% of the time, and significantly better than first-order linear model #% of the time.
The non-linearity was rarely pronounced... 
