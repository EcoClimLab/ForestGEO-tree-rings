---
title: "Supplementary Information: Joint effects of climate, tree size, and year on annual tree growth derived from tree-ring records of ten globally distributed forests (Anderson-Teixeira et al., *GCB*)"
author:
output: 
  pdf_document:
    fig_caption: yes
...
# climwin plots  (turned off automatic transfer shortly prior to re-submission (8/31/21) because we had to manually edit plots)
...
### Reconstructing diameter at breast height (DBH)
...
Here, $r_{bark}$ denotes bark thickness, which was estimated on a species-specific basis from allometries (detailed below; Tables `r species_table`, `r bark_allometries`).
...
Again, $r_{bark}$ was estimated on a species-specific basis from allometries (detailed below; Tables `r species_table`, `r bark_allometries`).
...
**Bark allometry field gives an ID number, corresponding to ID numbers in Table `r bark_allometries`, for the allometric equation used to estimate bark thickness. When neither raw data nor an allometric equation for the study species was available, we selected the most appropriate equation that could be located for similar species. 
...
Site and species codes are given in tables `r sites` and `r species_table`, respectively. Shown are numbers of trees and cores sampled and numbers for which DBH could be reconstructed (as described in Appendix `r `DBH_reconstruction`); ranges of tree sizes, in diameter at breast height (DBH; cm), as sampled and as reconstructed; date ranges analyzed; and whether or not data for the species met our criteria for analysis of year effects. 
...
library(knitr)
library(kableExtra)
allometries_table <- read.csv("/Users/kteixeira/Dropbox (Smithsonian)/GitHub/EcoClimLab/ForestGEO_dendro/data_processed/dbh_to_bark_allometries_table.csv",
                              col.names = c("ID", "species", "equation", "source type", "n", "DBH range (cm)", "sampling location", "source"),
                              stringsAsFactors = FALSE, check.names = FALSE)
...
```
Shown are allometric equations relating bark thickness (mm; *bark.mm*) to tree stem diameter at breast height (cm; *dbh.cm*). Equations were obtained from previous studies (source type = equation) or derived here from raw data (source type = data) as described in Appendix `r DBH_reconstruction`. For assignments of these allometries to the species included in this study, see Table `r species_table`.
...
PPT = precipitation, PET = potential evapotranspiration. NA= not applicable (has not been analyzed).
...
![ ](tables_figures/SI_figures/traditional_comparison/climwin_vs_dcc_SCBI_LITU_pre.png){ width=90% }

**Potential Evapotranspiration**

![](tables_figures/SI_figures/traditional_comparison/climwin_vs_dcc_SCBI_LITU_pet.png){ width=90%}
...
![ ](tables_figures/SI_figures/traditional_comparison/climwin_vs_dcc_Zofin_ABAL_wet.png){ width=90% }

**Maximum temperature**

![ ](tables_figures/SI_figures/traditional_comparison/climwin_vs_dcc_Zofin_ABAL_tmx.png){ width=90% }
...
![ ](tables_figures/SI_figures/traditional_comparison/climwin_vs_dcc_CedarBreaks_PSME_pre.png){ width=90% }

**Maximum temperature**

![](tables_figures/SI_figures/traditional_comparison/climwin_vs_dcc_CedarBreaks_PSME_tmx.png){ width=90% }
...
![ ](tables_figures/SI_figures/traditional_comparison/climwin_vs_dcc_ScottyCreek_PIMA_pre.png){ width=90% }

**Maximum temperature**

![](tables_figures/SI_figures/traditional_comparison/climwin_vs_dcc_ScottyCreek_PIMA_tmx.png){ width=90% }
...
![`r GLS_BCNM_legend` Shown are responses for all three growth metrics examined here: ring width (RW), basal area increment (BAI), and aboveground biomass growth ($\Delta$AGB). `r GLS_generic_legend` PPT= precipitation.](tables_figures/SI_figures/composite_plots/BCI.png){ width=80% }


\newpage
## `r GLS_SCBI_legend`

![`r GLS_SCBI_legend` Shown are responses for all three growth metrics examined here: ring width (RW), basal area increment (BAI), and aboveground biomass growth ($\Delta$AGB). `r GLS_generic_legend`  PPT= precipitation, PET = potential evapotranspiration.](tables_figures/SI_figures/composite_plots/SCBI.png){ width=80% }

\newpage
## `r GLS_SCBI_year_legend`

![`r GLS_SCBI_year_legend` Shown are responses for all three growth metrics examined here: ring width (RW), basal area increment (BAI), and aboveground biomass growth ($\Delta$AGB). `r GLS_generic_legend` PPT= precipitation, PET = potential evapotranspiration.](tables_figures/SI_figures/composite_plots/SCBI_with_Year.png){ width=80% }
...
![`r GLS_LT_year_legend` Shown are responses for all three growth metrics examined here: ring width (RW), basal area increment (BAI), and aboveground biomass growth ($\Delta$AGB). `r GLS_generic_legend` PPT= precipitation.](tables_figures/SI_figures/composite_plots/NewMexico_with_Year.png){ width=80% }
