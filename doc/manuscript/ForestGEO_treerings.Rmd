...
Within warmer regions, forests in humid climates can sometimes benefit from warm winter or spring temperatures [@babst_twentieth_2019; @tumajer_increasing_2017], as we show for all three species at ZOF and one of our species at HF (Fig. `r clim_sens`).
However, the predominantly negative temperature responses (Fig. `r clim_sens`) imply that in most forests, tree growth is likely to be reduced by warming temperatures.
...
An analytical structure that can account for this and other such DBH-climate interactions [e.g., @rollinson_climate_2021; @rossi_age-dependent_2007] will be critical to using tree-ring records to understand and forecast the effects of climate on tree growth and forest productivity.
We note that a modification of our analysis method (modeling $year \times$ climate interactions instead of $climate \times$ climate interactions) could potentially be used to account for directional changes in climate sensitivity, which have commonly been documented [e.g., @helcoski_growing_2019; @maxwell_declining_2016; @peltier_tree_2020; @zuidema_recent_2020].
...
Declines in $BAI$ and $\Delta AGB$ at larger $DBH$ are probably in part attributable to increasing allocation to reproduction [@meinzer_age-related_2011], and are also linked to slowly changing environmental conditions (e.g., successional changes in stand structure, climate change).
...
However, our results remain subject to some potential sampling and survivorship biases that can influence overall trends in positive or negative directions [Fig. `r year_fig`, @brienen_detecting_2012; @groenendijk_no_2015; @brienen_tree_2017; @nehrbassahles_influence_2014].
...
In secondary stands, particularly those where many of the sampled species recruited in pulses that were followed by low recruitment (e.g., SCBI, HF; Appendix `r site_details_appendix`, Figs. `r all_recruitment_plots`), growth declines are consistent with the tendency for faster tree growth during early succession (**REFS**), and with increasing competition and declining woody productivity as young stands mature [e.g., @goulden_patterns_2011; @pregitzer_carbon_2004].
