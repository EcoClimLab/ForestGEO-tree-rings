...
For instance, the additive or interactive effects of precipitation and temperature are not commonly considered [but see e.g., @foster_predicting_2016; @meko_seascorr_2011;  @sanchez-salguero_disentangling_2015; @vlam_temperature_2014; @zuidema_recent_2020].
Specifically in the case of drought, soil moisture (shaped by precipitation and evapotranspiration) and atmospheric vapor pressure deficit (VPD; driven by temperature) affect plant growth differently [@novick_increasing_2016; @humphrey_soil_2021], and we hypothesize that they commonly influence growth over different seasonal windows (Table `r hypotheses`).
...
Despite the fundamental importance of the relationship between DBH and growth, there remain inconsistencies as to what is considered a typical growth pattern. 
In dendrochonology, the "textbook" pattern of radial stem growth increments (ring width, RW) is one of rapid initial growth followed by a slow decline [@fritts_tree_1976], whereas forest census data reveals a pattern of increasing RW with DBH across globally distributed forests [@muller-landau_testing_2006; @anderson-teixeira_sizerelated_2015].
...
In addition, dendrochonology studies often observe or assume that basal area increment (BAI) is constant with DBH after a juvenile growth phase [@fritts_tree_1976; @biondi_theorydriven_2008] -- a pattern that we would not expect to hold in understory-established trees (Table `r hypotheses`).
...
A solid understanding of the ontogeny of tree growth is essential not only for predicting the growth rate of any given tree, but also for standardizing for DBH to deduce the influence of slowly changing environmental drivers [see next paragraph, @peters_detecting_2015], with the reliability of such analyses contingent upon accurate assumptions of ontogenetic growth patterns.

Beyond the direct effects of climate, rising atmospheric CO~2~ concentrations, and changes in atmospheric deposition of sulfur dioxide (SO~2~) and nitrogen oxides (NO~x~), and indirect effects of climate change are all potentially influencing tree growth [@levesque_water_2017; @mathias_disentangling_2018; @maxwell_higher_2019; @walker_integrating_2020; @takahashi_air_2020; @belmecheri_precipitation_2021], and understanding these effects is central to predicting the future of the terrestrial carbon sink [@walker_integrating_2020].
...
Ontogenetic patterns in tree growth must be accounted for, yet two of the most commonly used methods of standardizing for tree size, conservative detrending and basal area correction [@peters_detecting_2015], assume certain growth patterns that may not be universal in forest settings. 
