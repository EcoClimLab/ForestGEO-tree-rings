...
hypotheses_table <- subset(hypotheses_table, select = -c(table_row, category, notes) )#remove columns that shouldn't be included
ft <- flextable(hypotheses_table)
ft <- autofit(ft)
ft <- footnote( ft, i = 1, j = 2,
                value = as_paragraph(
                  c("SSC= species-site combinations; SSCV=species-site-variable combination")
                ),
                ref_symbols = c("*"),
...
For instance, ... *give an example/more clear explanation here? (see Camille's feedback)*
