...
Within the GLS models, our response variables were $log[\Delta RW]$, $log[BAI]$, or $log[\Delta AGB]$. 
...
To verify that GLS model trends for yeaar were real, as opposed to an artifact of inherent covariation between DBH and yeaar within each core, we compared GLS results to an analysis of DBH-growth relationships by decade (Figs. `r decadal_analysis_figs_all`). 
...
Within each of three categories of models run (climate only, climate + DBH, climate $\times DBH$, climate + DBH + yeaar), we selected as the top model that with the lowest AIC.  
...
In cases such as this, a non-linear response function to yeaar, or incorporation of data on pollution, would be needed to accurately capture changing growth trends, but that is beyond the scope of the current analysis.
