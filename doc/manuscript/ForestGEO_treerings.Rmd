...
While multifactorial and sometimes non-linear individual-based analysis frameworks have been applied in tree ring analysis [e.g., @evans_fusing_2017; @zuidema_recent_2020; @rollinson_climate_2021], their use has been relatively limited, and none simultaneously account for climate, DBH, and year.
...
The classic dendrochronological approach to characterizing the climate sensitivity of tree growth describes linear relationships between the main growth-limiting climate factor (moisture or temperature) and population-level growth responses captured in ring-width index chronologies [@fritts_tree_1976; @speer_fundamentals_2010].
While invaluable for applications such as reconstructing past climates [e.g.,  @buntgen_2500_2011], more in-depth characterization of climate sensitivity across a representative sample of all trees in a forest stand is desirable for efforts to understand forest productivity and its climate responses [@babst_when_2018].
First, the additive or interactive effects of moisture and temperature are not commonly considered [but see e.g., @foster_predicting_2016; @meko_seascorr_2011;  @sanchez-salguero_disentangling_2015; @vlam_temperature_2014; @zuidema_recent_2020].
While drought metrics such as the Standardised Precipitation-Evapotranspiration Index [SPEI, @vicente-serrano_multiscalar_2010] integrate effects of precipitation and temperature, soil moisture and atmospheric vapor pressure deficit (VPD) affect plant growth differently [@novick_increasing_2016], and we hypothesize that they commonly influence growth over different seasonal windows (Table `r hypotheses`).
In addition, based on the predominance of nonlinear climate responses of tree growth over both smaller and larger spatiotemporal scales [e.g., @kumarathunge_acclimation_2019; @banburymorgan_global_2021], we hypothesize that nonlinear climate responses, already known to be widespread within forest settings [@woodhouse_artificial_1999; @wilmking_global_2020; @rollinson_climate_2021], are in fact the predominant form of response in forests around the world (Table `r hypotheses`). 
Finally, while the influence of DBH is typically removed through detrending [@cook_calculating_1997], thereby eliminating the potential to directly model its influence on climate sensitivity, we hypothesize that interactive effects of DBH and climate are, in fact, quite common in forest settings [Table 1, @trouillier_size_2019; @bennett_larger_2015; @mcgregor_tree_2020; @rollinson_climate_2021].

Tree stem diameter, DBH, scales predictably with numerous traits affecting tree physiology [e.g., height, crown size and position, root mass, hydraulic architecture, @enquist_global_2002; @niklas_plant_2004; **REF**] and therefore is itself linked to growth [e.g., @muller-landau_testing_2006; @foster_predicting_2016] and its climate sensitivity [e.g., @bennett_larger_2015; @mcgregor_tree_2020].
...
Despite the fundamental importance of a solid understanding of the relationship between DBH and growth, there remain inconsistencies as to what is considered a typical growth pattern. 
