...
Note: This analysis lives in a GitHub repository (currently private): https://github.com/EcoClimLab/ForestGEO-climate-sensitivity. You're all welcome to see full code & results there, and to raise/ comment on [issues](https://github.com/EcoClimLab/ForestGEO-climate-sensitivity/issues). Ask KAT for access (you'll need a GitHub profile).
...
R script is here: https://github.com/EcoClimLab/ForestGEO-climate-sensitivity/blob/master/scripts/Analysis.R. (ask KAT for access if you can't access)
...
1.1. "Detrend" individual tree growth records ($log[radial.growth.increment]$ or $log[\Delta AGB]$) by fitting a spline using a GAM, thereby producing residuals. This parallels detrending of traditional dendro analyses, which seems necessary to pull out a good climate signal (see "Considerations and alternative approaches"). However, rather than averaging by species as per traditional dendro methods, we will analyze individual-level residuals.

1.2. We use climate data from CRU v. 4.04. Based on how variables are related to one another, and how they are derived in CRU ([Harris et al. 2020](https://rdcu.be/b3U3e)), we grouped variables as follows, using climwin to select the best variable and time window for each: 
- Precip group (describes precip and its frequency): PRE, WET
- temperature group (describes temperature): TMP, TMIN, TMAX, PET
- temperature range group (describes variation in temperature, linked to cloud cover): DTR, CLD, PET

1.3. Use the [climwin](https://cran.r-project.org/web/packages/climwin/index.html) R package [(van de Pol et. al. 2016)](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.12590), to identify the climate variables and time windows that most strongly influences tree growth *for all species combined*. 
Climwin can check for linear and quadratic relationships. We checked quadratic only because (i) it speeds up the process, (ii) preliminarly tests revealled that quadratic was usually the one with lower AIC, and when not there tended to be little difference in AIC and the curve would look like a straight line anyway, (iii) quadratic makes more biological sense, (iv) it would be too tricky to code the gam to adapt to any circumstance.
We put the residuals derived in step 1.1 in *climwin* with a mixed effects model using species and tree identity as random effect: `residual ~ [climate] + (1 | sp) + (1 | treeID)`. 
Outputs are saved [here](https://github.com/EcoClimLab/ForestGEO-climate-sensitivity/blob/master/results/ALL_species_mixed_model_on_residuals/). Example output is shown in Fig. 1.
...
1.3. For each group of candidate climate variables, we move forward with the best variable over the time window identified by climwin as a candidate climate variable for the multivariate models. When PET comes out as the best variable in both temperature and temperature range groups (should always be for the same time frame), then there are only 2 candidate variables for the GAM. 
...
2.1. The 2-3 candidate variables for the GAM should then go through the same process of checking for collinearity. If there is redundancy, we removed any variable with VIF > 10 (more standard way than looking at AIC).

2.2. For each species independently, run GAM models including every combination of climate variables (as a 2-degree polynomial to allow quadratic curve), spline of DBH (3 knots), and splines of year fit to each individual as a random effect.
`ln[response variable] ~ spline(DBH) + quad([climate variables]) + spline(year, with treeID as random effect)`

2.3. Across all possible models, sum variable AICc weights to determine which drivers are most important. Consider the model containing those variables as the top model (even if it's not top in terms of AIC)

2.4. Plot best model for each species (Figs. 2-3)

![**Fig. 2. OUTDATED Current SCBI output for radial increment**. For each species, relationships are plotted if included in top model. Grey bars indicate long-term mean climate, shading indicates 1 SD. Time windows for each climate variable (CRU abbreviations) are indicated above plot.](figures/ALL_variables_log_core_measurement.png)


![**Fig. 3. OUTDATED Current SCBI output for $\Delta AGB$**. For each species, relationships are plotted if included in top model. Grey bars indicate long-term mean climate, shading indicates 1 SD.  Time windows for each climate variable (CRU abbreviations) are indicated above plot. ](figures/ALL_variables_log_agb_inc.png)
...
- **ring width (radial growth)** - Annual ring width from individual-level chronologies. Helene Muller-Landau argues that, from tree scaling/geometry perspective, this metric is the closest to being size-independent (as opposed to BAI) (see [Francis et al. (2017)](https://onlinelibrary.wiley.com/doi/full/10.1111/geb.12604)). Empirically, this seems to be the case in closed-canopy broadleaf forests.
...
- **basal area increment (BAI)** - (Currently don't plan to use this.) BAI is commonly considered by dendrochronologists to be independent of DBH (or at least moresy than ring width) after trees pass a juvenile growth phase, the logic being that if a tree produces a constant amount of stem wood area each year, BAI will hold steady and ring width will decline. However, while this may hold true in some cases, evidence from closed-canopy broadleaf forests around the world (ForestGEO) suggests that radial/ diameter growth *increases* with DBH (e.g., Muller-Landau et al. 2006, [Anderson-Teixeira et al. 2015](https://www.researchgate.net/publication/277918165_Size-related_scaling_of_tree_form_and_function_in_a_mixed-age_forest)). There are some likely differences between trees represented by tree-rings (survivors) and the community as a whole, but in [Helcoski et al. 2019](https://nph.onlinelibrary.wiley.com/doi/abs/10.1111/nph.15906) we show a positive relationship of radial growth to DBH from tree-rings. The implication--based on basic geometry--is that BAI increases even more strongly with DBH than does radial growth hence should not be considered independent of DBH. Furthermore, BAI in itself is not very meaningful, as wood is added in three dimensions, and BAI only accounts for two. Thus, KAT sees no *a priori* motivation to use BAI over ring width or biomass. BAI was used by and Alexander et al. (in review) but without assumption that BAI is independent of DBH. 
...
- Should we account for age too? Probably. See [issue #21](https://github.com/EcoClimLab/ForestGEO-climate-sensitivity/issues/21). 
