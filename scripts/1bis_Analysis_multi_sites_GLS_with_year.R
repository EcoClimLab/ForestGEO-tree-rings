...
## Run the Analysis ####
for(solution_to_global_trend in c("none", "detrend_climate", "use_only_older_records")[c(1)]) {
  ...
  for(site in switch(solution_to_global_trend, "none" = sites, c("ScottyCreek", "NewMexico", "SCBI"))) {
    ...
    for(what in switch(as.character(any(!is.na(all_Biol[[site]]$dbh))), "TRUE" = c("log_core_measurement", "log_core_measurement_dbh", "log_agb_inc_dbh", "log_BAI_dbh"), "FALSE" = "log_core_measurement")) {
      ...
      # now do a species by species gls using log of raw measurements, spline on dbh and year (WITH AND WITHOUT year) ####
      
      for(with_year in switch(solution_to_global_trend, "none" = c(TRUE), FALSE)) {
        ## look at collinearity between climate variables ( and year and dbh when relevant) and remove any variable with vif > 10 ####
        if(grepl("dbh", what) & with_year) X <- Biol[, c(as.character(best_results_combos$climate), "Year", "dbh")]
        if(grepl("dbh", what) & !with_year) X <- Biol[, c(as.character(best_results_combos$climate), "dbh")]
        if(!grepl("dbh", what) & with_year) X <- Biol[, c(as.character(best_results_combos$climate), "Year")]
        if(!grepl("dbh", what) & !with_year) X <- Biol[, c(as.character(best_results_combos$climate))]
        ...
        sink(paste0("results/", ifelse(with_year, "with_year/", ""), ifelse(detrended, "with_detrended_climate/", ""), ifelse(old_records_only, "old_records_only/", ""), what, "/", site, "/VIF.txt"))
        print(vif_res)
        sink()
        ...
        # remove second order term for Year
        full_model_formula <- gsub("I\\(Year\\^2\\) \\+ ", "", full_model_formula)
        sexpr <- parse(text =gsub("\\| \\(`I\\(Year\\^2\\)` & \\!Year\\)", "", sexpr))
        ...
        for(sp in unique(Biol$species_code)) {
          ...
          # save dd
          write.csv(dd, file = paste0("results/", ifelse(with_year, "with_year/", ""), ifelse(detrended, "with_detrended_climate/", ""), ifelse(old_records_only, "old_records_only/", ""), what, "/", site, "/", sp, "_model_comparisons.csv"), row.names = F)
          ...
          if(grepl("dbh", what) & !with_year & "dbh" %in% variables_to_keep & !detrended & !old_records_only) {
            ...
            for(g in names(clim_var_group)) {
              ...
              if(length(v_int) > 0 ) {
                ...
                climate_interactions <- 
                  rbind(climate_interactions,                                         data.frame(with_year,
                                                                                                 what,
                                                                                                 site,
                                                                                                 species = sp,
                                                                                                 climate_group = g,
        ...
        # save the plot
        png(paste0('results/', ifelse(with_year, "with_year/", ""), ifelse(detrended, "with_detrended_climate/", ""), ifelse(old_records_only, "old_records_only/", ""), what, "/", site, "/GLS_Sum_of_AICweights_", site, '.png'),
            width = 10,
            height =8,
            units = "in",
            res = 300)
        ...
        for(v in c( variables_to_keep)) { 
          ...
          # if(!with_year) ylim_p[[what]][[v]][[site]] <- range(pt$lwr, pt$upr)
        }
        ...
        # save plot
        png(paste0('results/', ifelse(with_year, "with_year/", ""), ifelse(detrended, "with_detrended_climate/", ""), ifelse(old_records_only, "old_records_only/", ""), what, "/", site, "/GLS_ALL_variables_responses_", site, '.png'),
            width = 10,
            height =8,
            units = "in",
            res = 300)
        ...
        save(list = grep("^p_|pt|clim_var_group$|ylim_p", ls(), value = T), file = paste0('results/', ifelse(with_year, "with_year/", ""), ifelse(detrended, "with_detrended_climate/", ""), ifelse(old_records_only, "old_records_only/", ""), what, "/", site, "/env.RData"))
  ...
  # VIF_files <- VIF_files[!grepl("year", VIF_files)]
  # 
